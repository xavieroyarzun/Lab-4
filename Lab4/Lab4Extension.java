...
class Hospital {
    ...
    public Hospital() {
        ...
        this.colaAtencion = new PriorityQueue<>((p1, p2) -> {
            int prioridadP1 = calcularPrioridadDinamica(p1);
            int prioridadP2 = calcularPrioridadDinamica(p2);
        ...
        this.tiempoInicioSimulacion = System.currentTimeMillis() / 1000;
    }

    private int calcularPrioridadDinamica(Paciente p) {
        long tiempoEspera = (System.currentTimeMillis() / 1000 - p.getTiempoLlegada()) / 60;

        int prioridadBase = p.getCategoria();

        int factorIncremento = 0;

        if (p.getCategoria() >= 4) {
            factorIncremento = (int) (tiempoEspera / 30);
        } else if (p.getCategoria() == 3) {
            factorIncremento = (int) (tiempoEspera / 60);
        }


        return Math.max(1, prioridadBase - factorIncremento);
    }
...
class SimuladorUrgencia {
    ...
    private Map<Integer, Integer> pacientesNoAtendidos;
    private Map<Integer, Long> maxTiempoEspera;
    ...
    public SimuladorUrgencia() {
        ...
        this.pacientesNoAtendidos = new HashMap<>();
        this.maxTiempoEspera = new HashMap<>();

        for (int i = 1; i <= 5; i++) {
            pacientesPorCategoria.put(i, 0);
            tiempoEsperaAcumulado.put(i, 0L);
            pacientesNoAtendidos.put(i, 0);
            maxTiempoEspera.put(i, 0L);
        }
    }
    ...
    private void atenderPaciente() {
        ...
        if (p != null) {
            ...
            if (tiempoEspera > maxTiempoEspera.get(p.getCategoria())) {
                maxTiempoEspera.put(p.getCategoria(), tiempoEspera);
            }
    ...
    private void registrarPacientesNoAtendidos() {
        List<Paciente> pacientesAtendidos = hospital.getPacientesAtendidos();

        for (Paciente p : pacientesGenerados) {
            if (!pacientesAtendidos.contains(p)) {
                pacientesNoAtendidos.put(p.getCategoria(), pacientesNoAtendidos.get(p.getCategoria()) + 1);
                long tiempoEspera = p.tiempoEsperaActual();
                if (tiempoEspera > maxTiempoEspera.get(p.getCategoria())) {
                    maxTiempoEspera.put(p.getCategoria(), tiempoEspera);
                }
    ...
    public void mostrarEstadisticas() {
        System.out.println("Total de pacientes generados: " + pacientesGenerados.size());
        ...
        for (int i = 1; i <= 5; i++) {
            System.out.println("C" + i + ": " + pacientesPorCategoria.get(i) +
                    " (No atendidos: " + pacientesNoAtendidos.get(i) + ")");
        }
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            System.out.printf("C%d: %.2f minutos (Máximo: %d)\n", i, promedio, maxTiempoEspera.get(i));
        }
